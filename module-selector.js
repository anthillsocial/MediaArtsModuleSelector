...
<head>
  ...
  <style>
    ...
    .module-card {
      ...
      padding: 0.10em 0.40em 0.08em 0.40em;
      cursor: pointer;
      position: relative;
...
<body>
  ...
  <script>
    // === Replace this with your own data if needed ===
    const csvData = `
...
`.trim();
    ...
    class ModuleSelector {
      ...
      // Group-logic utility: returns all codes in this group (including self)
      findGroupMembers(code) {
        const groupCodes = new Set([code]);
        const thisMod = this.codeMap[code];
        this.modules.forEach(m => {
          if (m.group.includes(code) || thisMod.group.includes(m.code)) {
            groupCodes.add(m.code);
            m.group.forEach(gc => groupCodes.add(gc));
          }
        });
      ...
      selectModule(code, recurse = false) {
        ...
        // Select all group members (including self)
      ...
      deselectModule(code, recurse = false) {
        if (!this.selected.has(code)) return;
        // Deselect all group members (including self)
        const allGroupMembers = this.findGroupMembers(code);
        allGroupMembers.forEach(c => {
          if (this.selected.has(c)) {
